...
class FacesDataset(Dataset):
    """Loads AgingGAN data"""
    def __init__(self, path, mode="conditional", size: int = 4992, transform=None):
        """
        ...
        self.young_img_names = [os.path.join(path, fname) for fname in os.listdir(path) if "jpg" in fname and
                                30 > int(fname.split("_")[0]) > 8][:size]
        self.old_img_names = [os.path.join(path, fname) for fname in os.listdir(path) if  "jpg" in fname and
                              int(fname.split("_")[0]) > 45][:size]
